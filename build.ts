...
`;
themeconf.forEach((theme) => {
  console.log("Generating theme: " + Bun.inspect(theme));
  let prevalence = "ThemeLight";
  if (theme.prevalence == "dark") {
    prevalence = "ThemeDark";
  }
  const font_size = (() => {
    const a = parseFloat(theme["font-size"]).toString();
    if (a.includes(".")) {
      return a;
    } else {
      return a + ".0";
  ...
  const font_size_mono = (() => {
    const a = parseFloat(theme["font-size-mono"]).toString();
    if (a.includes(".")) {
      return a;
    } else {
      return a + ".0";
